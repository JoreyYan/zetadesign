...
def from_pdb_string(pdb_str: str, chain_id: Optional[str] = None) -> Protein:
    ...
    if '1' in chain_ids:
        chain_ids=['A']*len(chain_ids)
    elif '2' in chain_ids:
        chain_ids=['B']*len(chain_ids)

    elif '3' in chain_ids:
        chain_ids=['C']*len(chain_ids)
    elif '4' in chain_ids:
        chain_ids=['D']*len(chain_ids)
    elif '8' in chain_ids:
        chain_ids=['H']*len(chain_ids)
    elif '10' in chain_ids:
        chain_ids=['J']*len(chain_ids)
    elif 'j' in chain_ids:
        chain_ids=['J']*len(chain_ids)
    elif 'a' in chain_ids:
        chain_ids=['A']*len(chain_ids)
    elif 'g' in chain_ids:
        chain_ids=['G']*len(chain_ids)
    elif ' ' in chain_ids:
        chain_ids=['X']*len(chain_ids)
    elif 'd' in chain_ids:
        chain_ids=['D']*len(chain_ids)
    elif '0' in chain_ids:
        chain_ids=['Z']*len(chain_ids)
    elif 'q' in chain_ids:
        chain_ids=['Q']*len(chain_ids)
    elif 'A' in chain_ids:
        chain_ids=['A']*len(chain_ids)
    else:
        chain_ids=['A']*len(chain_ids)
    ...
    # atom_mask=residue_constants.STANDARD_ATOM_MASK[aatype]  #atom_mask*
    return Protein(
...
def to_pdb_ca(prot: Protein) -> str:
    """Converts a `Protein` instance to a PDB string.
    ...
    atom_mask = prot.atom_mask[:,1,None]
    aatype = prot.aatype
    atom_positions = prot.atom_positions[:,1,None,:]
    residue_index = prot.residue_index.astype(np.int32)
    b_factors = prot.b_factors[:,1,None]
    chain_index = prot.chain_index
