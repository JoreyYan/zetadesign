...
class InvariantPointAttention(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.to_outss = nn.Linear(heads * (scalar_value_dim + pairwise_repr_dim + point_value_dim * (3 + 1)), dim)
        ...
        **kwargs
        ...
        for _ in range(depth):
            self.layers.append(nn.ModuleList([
                IPABlock(dim = dim,require_pairwise_repr=False, **kwargs),
                nn.Linear(dim, 6)
            ]))
        ...
        if predict_points:
            self.Ca_to_points = nn.Linear(dim, 3)
            self.N_to_points = nn.Linear(dim, 3)
            self.O_to_points = nn.Linear(dim, 3)
            self.C_to_points = nn.Linear(dim, 3)
    ...
    def forward(
        ...
        *,
        ...
        for block, to_update in self.layers:
            ...
            quaternions = quaternion_multiply(quaternions, quaternion_update)  # this is same with Rigid compose_q_update_vec
        ...
        # Ca_points_local = self.Ca_to_points(x)
        N_points_local = self.N_to_points(x)
        C_points_local = self.C_to_points(x)
        O_points_local = self.O_to_points(x)

        rotations = quaternion_to_matrix(quaternions)
        Ca_points_global = translations   #einsum('b n c, b n c d -> b n d', Ca_points_local, rotations) +
        N_points_global = einsum('b n c, b n c d -> b n d', N_points_local, rotations) + translations
        C_points_global = einsum('b n c, b n c d -> b n d', C_points_local, rotations) + translations
        O_points_global = einsum('b n c, b n c d -> b n d', O_points_local, rotations) + translations

        XYZ=torch.stack((N_points_global,Ca_points_global,C_points_global,O_points_global),dim=-2)


        return XYZ,x, translations, quaternions
...
class IPA_Stack(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        self.layers=nn.ModuleList([
                IPABlock(dim = dim,**kwargs) for _ in range(depth)
            ])
    ...
    def forward(
        ...
        for block in self.layers:
            rotations = rigids.get_rots().get_rot_mats()
            translations=rigids.get_trans()
            ...
            # update quaternion and translation
            rigids = rigids.compose_q_update_vec(self.bb_update(x))
